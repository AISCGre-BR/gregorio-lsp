//! Gregorio LSP server (tower-lsp over stdio).
...
use gregorio_core::note_ops::{fill_empty_groups, shift_notes, ShiftDirection};
use gregorio_core::parser::types::Severity as PSeverity;
use gregorio_core::parser::GabcParser;
#[cfg(feature = "tree-sitter")]
use gregorio_core::tree_sitter_integration::TreeSitterParser;
#[cfg(feature = "tree-sitter")]
use gregorio_core::Position as GPosition;
#[cfg(feature = "tree-sitter")]
use gregorio_core::Range as GRange;
#[cfg(feature = "tree-sitter")]
...
fn to_diagnostic(err: gregorio_core::parser::types::ParseError) -> Diagnostic {
...
impl LanguageServer for Backend {
    ...
    async fn did_close(&self, params: DidCloseTextDocumentParams) {
        ...
        #[cfg(feature = "tree-sitter")]
        self.ts_trees.lock().unwrap().remove(&uri);
        self.client.publish_diagnostics(uri, Vec::new(), None).await;
    }
    ...
    async fn code_action(&self, params: CodeActionParams) -> Result<Option<CodeActionResponse>> {
        ...
        for diag in &params.context.diagnostics {
            let Some(data) = diag.data.as_ref() else { continue };
            let Some(fix) = data.get("fix") else { continue };
            ...
            changes.insert(
                params.text_document.uri.clone(),
                vec![TextEdit { range: edit_range, new_text: new_text.to_string() }],
            );
            actions.push(CodeActionOrCommand::CodeAction(CodeAction {
