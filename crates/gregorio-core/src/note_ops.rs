...
/// Replaces æ/ǽ/œ ligature characters with their `<sp>` tag equivalents.
pub fn ligatures_to_tags(text: &str) -> String {
    text.replace('æ', "<sp>ae</sp>")
        .replace('ǽ', "<sp>'ae</sp>")
        .replace('œ', "<sp>oe</sp>")
}
...
/// Returns the byte offset of the first character after the `%%` header/body
/// separator, or `0` when no separator is found.
pub fn body_start_byte(text: &str) -> usize {
    if let Some(idx) = text.find("%%") {
        // Skip past `%%` and the trailing newline (if present).
        let after = idx + 2;
